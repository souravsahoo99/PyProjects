...
def cross(series_a, series_b):
    ...
    return cross_over(series_a, series_b) or cross_under(series_a, series_b)
...
def inside_bar(high, low):
    ...
    if None in (h0, h1, l0, l1):
        return False

    return h0 <= h1 and l0 >= l1
...
def pivot_high(high, left, right):
    ...
    for i in range(1, left + 1):
        ...
        if _safe(high, center + i) is None:
            return None

        if _safe(high, center + i) >= pivot:
            return None
...
def pivot_low(low, left, right):
    ...
    for i in range(1, left + 1):
        ...
        if _safe(low, center + i) is None:
            return None

        if _safe(low, center + i) <= pivot:
            return None
